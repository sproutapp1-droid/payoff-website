      ...
      <div className="relative max-w-6xl mx-auto bg-primary/5 rounded-3xl py-16 px-6 sm:px-12 overflow-hidden">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-extrabold text-foreground">
            {hw.title || "Get started in 60 seconds"}
          </h2>
        </motion.div>

        <div className="relative grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="hidden md:block absolute top-[72px] left-[16.67%] right-[16.67%] border-t-2 border-dashed border-secondary/30" aria-hidden="true" />
          {steps.map((step: { number: string; title: string; description: string }, index: number) => (
            ...
            >
              <span className="text-secondary font-extrabold text-6xl leading-none">{step.number}</span>
